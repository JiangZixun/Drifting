...
DEFAULT_CONFIG_PATH = Path(__file__).resolve().parent / "configs" / "unet_drifting_target_ratio_sweep.json"
...
def train_step(
    ...
    *,
    ...
    def loss_fn(params):
        ...
        if drift_mode == "fixed_weight":
            drift_scale = jnp.asarray(drift_weight, dtype=jnp.float32)
        elif drift_mode == "target_ratio":
            drift_scale = (
                jnp.asarray(drift_target_ratio, dtype=jnp.float32)
                * jax.lax.stop_gradient(seg_loss)
                / (jax.lax.stop_gradient(drift_total) + 1e-6)
            )
        else:
            raise ValueError(f"Unsupported drift_mode={drift_mode!r}")

        drift_contrib = drift_scale * drift_total
        total = seg_loss + drift_contrib
        drift_ratio = drift_contrib / jnp.maximum(total, 1e-8)
        metrics = {
            **seg_metrics,
            "loss": total,
            "loss_seg": seg_loss,
...
def run_train_epoch(loader, state, step_fn, *, split, cfg, positive_banks, negative_banks):
    ...
    for batch in iterator:
        ...
        state, metrics, preds, prototypes, valids = step_fn(
            ...
            drift_mode=drift_cfg.get("mode", "fixed_weight"),
            drift_target_ratio=drift_cfg.get("target_ratio", 0.2),
            bank_ready=ready,
...
def build_parser():
    parser = argparse.ArgumentParser(description="Cloud segmentation with U-Net backbone and target-ratio drifting supervision.")
...
def main():
    ...
    cfg["drift"]["mode"] = "target_ratio"
    if args.target_ratio is not None:
        cfg["drift"]["target_ratio"] = float(args.target_ratio)
    else:
        cfg["drift"]["target_ratio"] = float(cfg["drift"].get("target_ratio", 0.2))
    wandb_cfg = dict(cfg.get("wandb", {}))
    wandb_cfg["name"] = Path(cfg["output_dir"]).name
    cfg["wandb"] = wandb_cfg
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        record["train"]["aux"]["drift_scale"] = float(train_metrics.get("drift_scale", record["train"]["aux"].get("drift_scale", 0.0)))
        record["train"]["aux"]["drift_contrib"] = float(train_metrics.get("drift_contrib", record["train"]["aux"].get("drift_contrib", 0.0)))
        record["train"]["aux"]["drift_ratio"] = float(train_metrics.get("drift_ratio", record["train"]["aux"].get("drift_ratio", 0.0)))
